export const CATEGORY = [
  { id: 'food', name: 'food' },
  { id: 'medical', name: 'medical' },
  { id: 'study', name: 'study' },
  { id: 'travel', name: 'travel' },
  { id: 'entertainment', name: 'entertainment' },
  { id: 'grocery', name: 'grocery' },
  { id: 'investment', name: 'investment' },
  { id: 'rent', name: 'rent' },
  { id: 'attire', name: 'attire' },
  { id: 'salary', name: 'salary' },
  { id: 'other', name: 'other' },
];

export const EXPENSE_FETCHED_SUCCESSFULLY = 'Expense fetched successfully.';
export const SOMETHING_WENT_WRONG = 'Something went wrong, please try again.';
export const UNAUTHORIZED =
  'Your session is expired, please login again, redirecting to the login page.';
export const EXPENSE_DELETED = 'Your expense deleted successfully';
export const DELETE_CONFIRM = 'Are you sure delete this task?';
export const DELETE_CONFIRM_DESC = 'Once deleted, cant be rolled back.';
export const DATA_UPDATED = 'Data updated successfully.';
export const EXPENSE_ADDED =
  'Expense added successfully, you can check it in the manage page.';
